...
describe('test', () => {
    it('test', async () => {
        let mqtt_canister = await context.deploy('./target/wasm32-unknown-unknown/release/container.wasm', {
    ...
    it('generate_candid', async () => {
        let mqtt_canister = await context.deploy('./target/wasm32-unknown-unknown/release/container.wasm') as WasmCanister
        let raw_candid = await mqtt_canister.get_candid();

        fs.writeFileSync('./container/src/mqtt.did', raw_candid)
    });
});
